...
@section('content')
    ...
    <div class="card card-custom">
        ...
        {!! Form::model($user,['route' => ['admin.users.update', Crypt::encrypt($user->id)], 'method' => 'PUT', 'id' => 'add_user']) !!}
			{{ csrf_field() }}
            <div class="card-body">
                <div class="row">
                ...
                <div class="form-group col-12">
                    ...
                    <select class="form-control select2" id="position" name="position_id">
                            ...
                            <option value="{{$positions->id}}"
                                @foreach ($position_selected as $selected)
                                @if($positions->id == $selected->id) selected @endif
                                @endforeach>{{$positions->name}}</option>
                            @endforeach
                    </select>
                </div>
